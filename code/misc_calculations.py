...
def plot_sites(sites, color='gray', ax=None):
    n = len(sites)
    x = np.zeros(n)
    y = np.zeros(n)
    for i in range(n):
...
def placement_symmetries_2(delta_x=50, delta_y=0):

    sites_2P_xs = np.linspace(1,3,3)
    sites_2P_ys_lower = np.linspace(0,1,2)
    sites_2P_ys_upper = np.linspace(2,3,2)
    sites_1P_xs = np.linspace(1,3,3)
    sites_1P_ys = np.linspace(1,2,2)
    ...
    plot_sites(sites_2P_upper, ax=ax)
    plot_sites(sites_2P_lower, ax=ax)
    plot_sites(sites_1P, ax=ax)
    ax.set_aspect(1)
    plt.show()
...
if __name__ == '__main__':
    ...
    placement_symmetries_2(delta_x = 10, delta_y = 0)
