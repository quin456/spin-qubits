...
A_2P_fab = np.array([36.6, 36.3, 35.7, 35.2, 34.9, 34.8, 34.5, 33.0, 35.8, 33.9]) * unit.MHz
pos_1P = np.array([
    [0,1,2,0,1,2],
    [0,0,0,1,1,1]
])
    ...
    for i2 in range(6):
        for i3 in range(6):
            ...
            if d3 not in A_dict.keys():
                A_dict[d3] = A_2P_fab[i_A]
                i_A+=1
            unique_configs[d_trip] = unique_configs.get(d_trip, []) + [[d1,d2,d3]]
            unique_hyperfines[f'{d3:.2f}'] = unique_hyperfines.get(f'{d3:.2f}', []) + [[i2,i3]]
...
J_rand = pt.rand(39) * (maxreal(J_100_110_18nm) - minreal(J_100_110_18nm)) + minreal(J_100_110_18nm)
