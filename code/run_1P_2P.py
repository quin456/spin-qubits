...
def run_2P_1P_CNOTs(tN,N, nS=15, Bz=0, max_time = 24*3600, save_data=False, lam=0, prev_grape_fn=None, verbosity=2):
    ...
    if prev_grape_fn is None:
        grape = GrapeESR_AJ_Modulation(J,A,tN,N, Bz=Bz, target=target, max_time=max_time, save_data=save_data, lam=lam, verbosity=verbosity)
    else:
        grape = load_grape(prev_grape_fn, max_time=max_time, Grape=GrapeESR_AJ_Modulation, verbosity=verbosity)
