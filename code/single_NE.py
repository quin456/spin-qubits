...
def get_coupling(A, Bz):
    ...
    K = (2 * (4 * A ** 2 + Gbar ** 2 - Gbar * np.sqrt(4 * A ** 2 + Gbar ** 2))) ** (
        -1 / 2
    )
...
def NE_transition_pulse(i, j, tN, N, A, Bz, ax=None, correct_phase=False):
...
def NE_CX_pulse(tN, N, A, Bz, ax=None, correct_phase=False):
    return NE_transition_pulse(2, 3, tN, N, A, Bz, ax, correct_phase)


def EN_CX_pulse(tN, N, A, Bz, ax=None, correct_phase=False):
    return NE_transition_pulse(1, 3, tN, N, A, Bz, ax, correct_phase)
...
def NE_swap_pulse(N_e, N_n, A, Bz, ax=None, correct_phase=False):

    Bx_NE, By_NE, T_e = NE_CX_pulse(None, N_e, A, Bz, correct_phase=correct_phase)
    Bx_EN, By_EN, T_n = EN_CX_pulse(None, N_n, A, Bz, correct_phase=correct_phase)
...
def print_specs(A, Bz, tN, N, gamma):
    ...
    print(
        f"Max allowed coupling = {max_coupling/unit.MHz} MHz, corresponding to field strength B={max_coupling/(0.5*gamma) / unit.mT} mT"
    )
...
def show_NE_CX(
    ...
    N,
    tN=None,
    psi0=(gate.spin_00 + gate.spin_10) / np.sqrt(2),
    fp=None,
    ax=None,
    fig=None,
    legend_loc="best",
    ...
    print(
        "Performing NE_CX, which flips electron spin conditionally on nuclear spin being down."
    )
    ...
    psi = pt.matmul(X, psi0)
    plot_psi(
        psi, tN=tN, T=T, ax=ax[2], label_getter=NE_label_getter, legend_loc=legend_loc
    )
    if fig is not None:
...
def get_phase_correction(Bz, A, Xf, N_search, tN_search, target=gate.CXr):
    ...
    print(
        f"Adding {tN_wait/unit.ns} ns free evolution to correct phase, achieving fidelity {fids[N_wait]}, up from {fids[0]}"
    )
...
def correct_phase(Bx_CX, By_CX, T_CX, N, Bz, A, target):
    ...
    Bx_wait, By_wait, T_wait = get_phase_correction(
        Bz, A, X_CX[-1], N, 500 * unit.ns, target=target
    )
    T = pt.cat((T_CX, T_CX[-1] + T_wait))
    Bx = pt.cat((Bx_CX, Bx_wait))
    By = pt.cat((By_CX, By_wait))
    print(
        f"Phase correction finished. Pulse now runs from {T[0]/unit.ns} ns to {T[-1]/unit.ns} ns"
    )
    return Bx, By, T
...
def show_EN_CX(
    ...
    N,
    tN=None,
    psi0=(gate.spin_00 + gate.spin_01) / np.sqrt(2),
    target=gate.CXr,
    fp=None,
    ax=None,
    fig=None,
    ...
    print(
        "Performing EN_CX, which flips nuclear spin conditionally on electron spin being down."
    )
...
def triple_NE_H0():
    return gamma_e * B0 * gate.get_Sz_sum(3) - gamma_n * B0 * gate.get_Iz_sum(
        3
    )  # unfinished
...
class NuclearElectronGrape(Grape):
    def __init__(
        ...
        self.nq = nq
        super().__init__(
            tN, N, target, rf, u0=u0, save_data=save_data, max_time=max_time
        )
...
def run_NE_grape():
    grape = NuclearElectronGrape(
        Bz=0.02 * unit.T, tN=100 * unit.ns, N=2500, target=gate.CXr, max_time=60
    )
...
def IP_NE_SWAP_things(A=get_A(1, 1), tN_e=100 * unit.ns, tN_n=12000 * unit.ns, N=20000):
    ...
    H_ac_IP = pt.einsum(
        "jac,jcd->jad", pt.einsum("jab,jbc->jac", dagger(U0), H_ac_IP), U0
    )
