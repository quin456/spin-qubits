...
from electrons import get_2E_H0, get_ordered_eigensystem, plot_free_electron_evolution, get_free_electron_evolution
...
def energy_level_picture(energies, state_labels=None, energy_labels=None, colors=colors, ax=None, fp=None):
...
def two_electron_energy_level_picture(ax=None, fp = None, detuning=True):

    if detuning:
        energies = [10, -1, -3.6, -10]
        labels = [f"$\omega_z+J$", f"$2A-J$",  f"$-2A-J$", f"$-\omega_z+J$"]
        state_labels = ["$|T_+⟩$", "$|T_0⟩$", "$|S_0⟩$", "$|T_-⟩$"]
    else:
        energies = [10, 1.2, -1.2, -10]
        labels = [f"$\omega_z+2A+J$", f"$J$",  f"-J", f"$-\omega_z-2A+J$"]
        state_labels = ["$|T_+⟩$", "$|T_0⟩$", "$|S_0⟩$", "$|T_-⟩$"]
    energy_level_picture(energies, state_labels=state_labels, energy_labels=labels, ax=ax, fp=fp, colors = ['blue', 'green', 'green', 'red'])
...
def triple_E_energy_level_picture(ax):
    ...
    energy_level_picture(energies, energy_labels=spin_labels, colors=colors, ax=ax)
...
def show_2E_Hw(J,A, tN, N, fp=None):
    ...
    S,D = get_ordered_eigensystem(A, J)
    U0 = get_U0(H0, tN, N)
...
def no_coupler():
    ...
    def triple_E_transitions(A=get_A(1,3), J=get_J(1,3),fp=None):
        fig,ax = plt.subplots(1,2)
        visualise_E_transitions(A=A, J=J, ax=ax[0])
        triple_E_energy_level_picture(ax=ax[1])
        fig.set_size_inches(10, 5)
        if fp is not None:
            fig.savefig(fp)
    
    def two_E_transitions(A=get_A(1,2), J=get_J(1,2), fp=None, detuning=True, figlabels=None):
        fig,ax = plt.subplots(1,2)
        ...
        if figlabels is not None:
            ax[0].annotate(figlabels[0], (-3,-8), fontsize=30)
            ax[1].annotate(figlabels[1], (-0,-8), fontsize=30)
        if fp is not None:
            fig.savefig(fp)
    ...
    def NE_swap_exchange_comparison(fp=None):
        ...
        N=500000
        double_NE_swap_with_exchange(J=get_J(1,2)/10, N=N, deactivate_exchange=True, ax=ax[0])
        double_NE_swap_with_exchange(J=get_J(1,2)/10, N=N, deactivate_exchange=False, ax=ax[1])
    ...
    # triple_E_transitions(fp = f"{plots_folder}NC-triple-E-transitions.pdf")
    # triple_E_transitions(A=get_A(1,3,NucSpin=[1,1,1]), J=get_J(1,3), fp=f"{plots_folder}NC-triple-E-transitions-no-detuning.pdf")
    # two_E_transitions(fp = f"{plots_folder}NC-allowed-2E-transitions.pdf", figlabels=['a','b'])
    # two_E_transitions(fp = f"{plots_folder}NC-allowed-2E-transitions-no-detuning.pdf", detuning=False, figlabels=['c', 'd'])
