...
def double_NE_swap_with_exchange(
    Bz=2 * unit.T,
    A=get_A(1, 1),
    J=get_J(1, 2),
    tN=500 * unit.ns,
    ...
    multi_NE_evol(
        ...
        J=J,
        psi0=gate.spin_0010,
        deactivate_exchange=deactivate_exchange,
        ax=ax,
...
def get_nuclear_spin_ordered_eigensystem(Bz=2 * unit.T, A=get_A(1, 1), J=get_J(1, 3)):
    ...
    def get_3NE_H0_for_ordering(Bz, A, J):
        ...
        H0 = (
            gamma_e * Bz * Sz
            - 0.5 * gamma_n * Bz * gate.IZIIII
            - 0.5 * gamma_n1 * Bz * gate.ZIIIII
            - 0.5 * gamma_n3 * gate.IIZIII
            + A * (o_n1e1 + o_n2e2 + o_n3e3)
            + J[0] * o_e1e2
            + J[1] * o_e2e3
        )
        return H0
...
def map_3NE_transitions():
    ...
    for i in idxs_100:
        for j in idxs_101:
            if (i, j) in trans:
                print(
                    f"{i}<-->{j} == {psi_to_string(S[:,i])} <--> {psi_to_string(S[:,j])} transition frequency = {pt.real(E[i]-E[j])/unit.MHz} MHz, coupling = {pt.real(C[i,j] * unit.T/unit.MHz)} MHz/T"
                )
            else:
...
def all_triple_NE_basis_transitions(
    tN=4000 * unit.ns, N=10000, Bz=2 * unit.T, A=get_A(1, 1), J=get_J(1, 3)
):
    ...
    for j, transition in enumerate(allowed_transitions):
        if transition[0] in basis and transition[1] in [i0, i1_target]:
            ...
            print(
                f"Transition |E{transition[0]}> <--> |E{transition[1]}> is allowed, w_res = {omega/unit.MHz} MHz"
            )
    for i in basis:
        triple_NE_estate_transition(i, tN, N, Bz, A, J, ax[i // 2, i % 2])


def get_NE_estate_transition(
    S, D, i_psi0=25, i_psi1=28, tN=4000 * unit.ns, N=10000, ax=None
):
    ...
    print(
        f"Target nuclear spin flip: w_res = {pt.real(omega)/unit.MHz} MHz, coupling = {pt.real(coupling)*unit.T/unit.MHz} MHz/tesla"
    )
    Bx, By = pi_pulse_square(omega, coupling, tN, N)
    return Bx, By


def triple_NE_estate_transition(
    i_psi0=27,
    i_psi1=28,
    tN=14000 * unit.ns,
    N=4000,
    ...
    Bx, By = get_NE_estate_transition(
        S, D, i_psi0=i_psi0, i_psi1=i_psi1, tN=tN, N=N, ax=ax
    )
    # Bx = None; By = None
...
def get_multi_NE_X_low_mem(
    ...
    for j in range(1, N):
        Hj = (
            H0
            + gamma_e * (Bx[j] * Sx + By[j] * Sy)
            - gamma_n * (Bx[j] * Ix + By[j] * Iy)
        )
...
def get_multi_NE_reduced_X_low_mem(
    Bx,
    By,
    Bz=2 * unit.T,
    A=get_A(1, 1),
    ...
    X_reduced = pt.zeros(
        N, dim_reduced, dim_reduced, dtype=cplx_dtype, device=default_device
    )
    for j in range(1, N):
        Hj = (
            H0
